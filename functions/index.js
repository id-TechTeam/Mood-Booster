...
//Connect to database
mongoose.connect(process.env.MONGO_URL || functions.config().someservice.id, { useNewUrlParser: true, useUnifiedTopology: true }, function (err) {
    if (err) {
...
app.use(
    session({
        secret: process.env.SESSION_SECRET || functions.config().session_secret.id,
        resave: false,
...
app.get('/contact', async (req, res) => {
    if (req.isAuthenticated()) {
        var _user = await User.findById(req.user._id);
    ...
    res.render("contact.ejs", { user: _user });
})
...
app.get('/about', async (req, res) => {
    ...
    res.render("about.ejs", { user: _user });
})
...
app.get('/mood/:userMood', async (req, res) => {
    ...
    res.render("healing/index.ejs", { mood: mood, user: _user });
})
...
app.get('/healing/:healing_method/:userMood?', async (req, res) => {
    ...
    switch (healing_method) {
        case "music":
            res.render('healing/music.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        case "video":
            res.render('healing/video.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        case "help":
            res.render('healing/help.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        case "workout":
            res.render('healing/workout.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        case "funny":
            res.render('healing/funny.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        case "quote":
            res.render('healing/quote.ejs', { mood: mood, healing_method: healing_method, user: _user });
            break;
        default:
            console.log("unrecognized healing method: " + healing_method);
            res.render('home.ejs', { user: _user });
    }
})
...
app.get("/users/login", checkNotAuthenticated, (req, res) => {
    var _user = new User();
    _user.isSignedIn = false;
    res.render("user/login.ejs", { user: _user });
});
...
app.get("/users/register", checkNotAuthenticated, (req, res) => {
    var _user = new User();
    _user.isSignedIn = false;
    res.render("user/register.ejs", { user: _user });
});
...
app.post("/users/register", checkNotAuthenticated, async (req, res) => {
    ...
    try {
        ...
        var _user = new User();
        _user.isSignedIn = false;
        res.render("user/register.ejs", { messages: { error: e }, user: _user });
    }
});
...
app.get('/getPicture/:selected', async (req, res) => {
    try {
        ...
        const config = {
            headers: {
                'Accept-Version': 'v1',
                Authorization: process.env.PIC_API_KEY || functions.config().pic_api_key.id
            }
        }
